/*
  ESP32 Traffic Light Controller with Updated LED Mapping
  -------------------------------------------------------
  Summary of changes: Updated LED mapping to reflect the new physical layout where each direction uses 2 specific NeoPixel indexes instead of contiguous blocks. LEDS_PER_DIRECTION set to 2, LED_COUNT set to 8. Added directionPixelMap for easy future remapping. Added a Serial.print at startup to show calculated normal RED duration (Total cycle - GREEN - YELLOW).

  All previous functionality (normal cycle, emergency override, NFC mapping, non-blocking millis() logic) preserved.
*/
...
// total_cycle = NUM_DIRECTIONS * (NORMAL_GREEN_MS + NORMAL_YELLOW_MS)
// red_time_per_direction = total_cycle - (NORMAL_GREEN_MS + NORMAL_YELLOW_MS)
...
static const uint8_t directionPixelMap[NUM_DIRECTIONS][LEDS_PER_DIRECTION] = {
  {1, 4}, // direction 0 (traffic light 1)
  {3, 6}, // direction 1 (traffic light 2)
  {0, 5}, // direction 2 (traffic light 3)
  {2, 7}  // direction 3 (traffic light 4)
};
...
void setup(void) {
  ...
  delay(100);
  Serial.println();
  Serial.println("=== ESP32 Traffic Light Controller (Updated LED Mapping) ===");
